...
public class ScanningHelper {
	 ...
	 */
	public static ArrayList<Vector3> findAllTileEntitesOf(World world, Class<? extends TileEntity> clazz, int x, int y, int z,
		...
		for (int ix = (x + (int) Math.floor(bounds.minX)); ix < (x + bounds.maxX); ix++)
			for (int iy = (y + (int) Math.floor(bounds.minY)); iy < (y + bounds.maxY); iy++)
				for (int iz = (z + (int) Math.floor(bounds.minZ)); iz < (z + bounds.maxZ); iz++) {
					TileEntity object = world.func_147438_o(ix, iy, iz);
	 ...
	 */
	public static TileEntity findNearestTileEntityOf(World world, Class<? extends TileEntity> clazz, int x, int y, int z,
			AxisAlignedBB bounds) {
		...
		for (Vector3 item : poolMatching)
			if (best.mag() > item.mag())
				best = item;
		return world.func_147438_o((int) Math.floor(x + best.x), (int) Math.floor(y + best.y), (int) Math.floor(z + best.z));
	}
