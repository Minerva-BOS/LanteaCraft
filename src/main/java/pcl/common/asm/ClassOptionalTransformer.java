...
public class ClassOptionalTransformer implements IClassTransformer {
	...
	public byte[] transform(String name, String transformedName, byte[] basicClass) {
		...
		if (classNode.visibleAnnotations != null)
			for (AnnotationNode a : classNode.visibleAnnotations) {
				...
				if (a.desc.equals("Lpcl/common/asm/ClassOptional$ClassFilterInterfaceList;")) {
					PCLCoreTransformerPlugin.getLogger().log(Level.DEBUG, "Found ClassFilterInterface annotation data, processing...");
				...
				if (n.visibleAnnotations != null)
					for (AnnotationNode a : n.visibleAnnotations)
						...
						if (a.desc.equals("Lpcl/common/asm/ClassOptional$ClassFilterMethod;")) {
							PCLCoreTransformerPlugin.getLogger().log(Level.DEBUG, "Found ClassFilterMethod annotation data, processing...");
		...
		if (ops > 0) {
			PCLCoreTransformerPlugin.getLogger().log(Level.DEBUG, String.format("Performing total %s ASM operations...", ops));
	...
	private void stripInterface(ClassNode classNode, String interfaceName) {
		...
		if (!found)
			PCLCoreTransformerPlugin.getLogger().log(Level.WARN,
					String.format("ClassOptional removal - interface %s NOT removed - interface not found", interfaceName));
	}
